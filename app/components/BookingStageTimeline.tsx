...
export const BOOKING_STAGES: Record<string, { key: string; label: string; icon: string; broadStatus?: string }[]> = {
  Surgery: [
    ...
    { key: "payment_done",       label: "Advance Payment Done",      icon: "💰" },
    { key: "admitted",           label: "Patient Admitted",          icon: "🛏️" },
    { key: "surgery_done",       label: "Surgery Done",              icon: "⚕️" },
    { key: "final_payment",      label: "Full & Final Payment",      icon: "💳" },
...
interface Props {
  ...
  onReschedule?: (newDate: string, newSlot: string, reason: string) => Promise<void>;
  readOnly?:     boolean;
}
...
export default function BookingStageTimeline({
  bookingId, type, currentStage, history, onUpdate, onReschedule, readOnly,
}: Props) {
  ...
  async function handleConfirm() {
    if (!notes.trim()) { setNotesErr("Comment likhna zaruri hai"); return; }
    if (!nextStage) return;
    setSaving(true);
    try {
  ...
  async function handleReschedule() {
    if (!rDate)         { setRErr("Nayi date select karein"); return; }
    if (!rReason.trim()) { setRErr("Reason likhna zaruri hai"); return; }
    setRSaving(true);
    try {
      await onReschedule!(rDate, rSlot, rReason.trim());
      setShowReschedule(false);
      setRDate(""); setRSlot(""); setRReason("");
    ...
    <div className="space-y-4">
      ...
      <div className={`rounded-2xl p-4 ${
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl ${
              currentStage === "cancelled" ? "bg-red-100" : "bg-white/20"
            }`}>
          ...
          <div className="flex items-center gap-3">
            {/* Reschedule button */}
            {!isFinal && (onReschedule || !readOnly) && (
              <button
                onClick={() => { setRDate(""); setRSlot(""); setRReason(""); setRErr(""); setShowReschedule(true); }}
        ...
        {!isFinal && nextStage && !readOnly && (
          <p className="text-white/70 text-xs mt-2">
            Next: <span className="text-white font-semibold">{nextStage.icon} {nextStage.label}</span>
          </p>
        )}
      </div>

      {/* ── Rescheduled banner (if history has reschedule entry) ── */}
      {histMap["rescheduled"] && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-start gap-3">
          <span className="text-xl flex-shrink-0">📅</span>
          <div className="text-xs space-y-0.5">
            <p className="font-bold text-amber-700">Booking Rescheduled</p>
            <p className="text-amber-600">👤 {histMap["rescheduled"].updatedBy} · 🕐 {fmtTime(histMap["rescheduled"].timestamp)}</p>
            {histMap["rescheduled"].notes && (
              <p className="text-amber-600 italic">💬 "{histMap["rescheduled"].notes}"</p>
            )}
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {stages.map((s, i) => {
          ...
          const isCancRow = currentStage === "cancelled" && i === curIdx;
          ...
          return (
            <div key={s.key} className={`flex gap-0 ${!isLast ? "border-b border-gray-50" : ""} ${
              active ? "bg-teal-50/40" : done ? "bg-white" : "bg-gray-50/40"
            }`}>
              {/* Left accent bar */}
              <div className={`w-1 flex-shrink-0 ${
                done        ? "bg-teal-400"
                : active    ? "bg-teal-500"
                : isCancRow ? "bg-red-400"
                : "bg-gray-100"
              }`} />

              <div className="flex gap-3 px-3 py-3 flex-1 min-w-0 items-center">
                ...
                <div className="flex-1 min-w-0 py-0.5">
                  <p className={`font-semibold text-sm leading-tight ${
                    done ? "text-teal-700" : active ? "text-gray-900" : "text-gray-300"
                  }`}>
                    {s.label}
                  </p>

                  {/* Audit trail for completed stage */}
                  {done && hist && (
                ...
                <div className="flex-shrink-0 pl-2">
                  {done && (
                    <button disabled
                      className="flex items-center gap-1 px-3 py-1.5 bg-green-50 text-green-600 border border-green-200 rounded-xl text-xs font-bold cursor-not-allowed select-none">
                      ✓ Completed
                    </button>
                  )}
                  {active && !readOnly && nextStage && (
                    <button onClick={openModal}
                      className="flex items-center gap-1.5 px-3 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-xl text-xs font-bold transition shadow-sm shadow-teal-200 active:scale-95 whitespace-nowrap">
                      Continue →
                    </button>
                  )}
                  {active && (readOnly || !nextStage) && (
                    <span className="text-[10px] bg-teal-100 text-teal-600 border border-teal-200 px-2.5 py-1.5 rounded-xl font-bold animate-pulse">
                      ● Active
                    </span>
                  )}
                  {future && (
                    <span className="text-[10px] text-gray-300 font-medium px-1">—</span>
                  )}
                </div>
              </div>
            </div>
        ...
        {currentStage === "cancelled" && (
          <div className="flex gap-0 bg-red-50">
            ...
            <div className="flex gap-3 px-4 py-3 flex-1 items-center">
              ...
              <div className="flex-1">
                ...
                {histMap["cancelled"] && (
                  <div className="mt-1 space-y-0.5 text-[11px] text-red-500">
                    <p>👤 {histMap["cancelled"].updatedBy} · 🕐 {fmtTime(histMap["cancelled"].timestamp)}</p>
                    {histMap["cancelled"].notes && <p>💬 "{histMap["cancelled"].notes}"</p>}
                  </div>
                )}
              </div>
              <button disabled className="px-3 py-1.5 bg-red-100 text-red-400 border border-red-200 rounded-xl text-xs font-bold cursor-not-allowed">
                Cancelled
              </button>
            </div>
          </div>
              ...
              <div className="px-6 py-5 space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={
                      nextStage.key === "hospital_confirmed"  ? "e.g. Hospital ne confirm kar diya, Dr. Sharma available hain"
                      : nextStage.key === "patient_confirmed" ? "e.g. Patient ko call kiya, 10 AM slot confirm hua"
                      : nextStage.key === "confirmed"         ? "e.g. Booking confirm ho gayi, SMS bheja gaya"
                      : nextStage.key === "pickup_scheduled"  ? "e.g. Pickup 7 AM ke liye schedule hua, driver ka no. diya"
                      : nextStage.key === "payment_done"      ? "e.g. ₹15,000 counter par jama hua, receipt no. 1234"
                      : nextStage.key === "final_payment"     ? "e.g. Full & final ₹25,000 payment clear hua, balance nil"
                      : nextStage.key === "surgery_done"      ? "e.g. Surgery successfully complete, Dr. Verma ne confirm kiya"
                      : nextStage.key === "discharged"        ? "e.g. Patient discharge summary diya gaya, cab arrange hua"
                      : "Detail likhein..."
                    }
                    rows={3}
                  ...
                  />
                  {notesErr && (
                    <p className="text-xs text-red-500 mt-1 flex items-center gap-1">⚠️ {notesErr}</p>
                  )}
                </div>
                <div className="flex gap-3 pt-1">
                  ...
                  <button onClick={handleConfirm} disabled={saving}
                    className="flex-grow bg-teal-600 hover:bg-teal-700 text-white py-3 px-6 rounded-xl text-sm font-bold transition disabled:opacity-50 flex items-center justify-center gap-2">
                    {saving
      ...
      {showReschedule && (
        <>
          <div className="fixed inset-0 bg-black/60 z-50 backdrop-blur-sm" onClick={() => setShowReschedule(false)} />
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
            <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden">
              <div className="bg-gradient-to-r from-amber-500 to-orange-500 px-6 pt-6 pb-5">
                <p className="text-amber-100 text-xs font-semibold uppercase tracking-wider mb-1">Booking Reschedule</p>
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center text-2xl">📅</div>
                  <div>
                    <p className="text-white font-black text-lg leading-tight">New Date &amp; Slot</p>
                    <p className="text-amber-200 text-xs mt-0.5 font-mono">{bookingId}</p>
                  </div>
                </div>
              </div>
              <div className="px-6 py-5 space-y-4">
                <div>
                  ...
                  <input type="date" value={rDate}
                    ...
                    onChange={e => { setRDate(e.target.value); setRErr(""); }}
                    className="w-full border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-200 focus:border-amber-400"
                  />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1.5">New Time Slot</label>
                  <input type="text" value={rSlot}
                    onChange={e => setRSlot(e.target.value)}
                ...
                <div>
                  ...
                  <textarea value={rReason}
                    onChange={e => { setRReason(e.target.value); if (e.target.value.trim()) setRErr(""); }}
                    placeholder="e.g. Patient ne request kiya — Doctor unavailable tha, naya date fix hua"
                    rows={3}
                    className={`w-full border-2 rounded-xl px-3 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 transition ${
                      rErr ? "border-red-300 focus:ring-red-200" : "border-gray-200 focus:ring-amber-200 focus:border-amber-400"
                    }`}
                  />
                  {rErr && (
                    <p className="text-xs text-red-500 mt-1 flex items-center gap-1">⚠️ {rErr}</p>
                  )}
                </div>
                <div className="flex gap-3 pt-1">
                  ...
                  <button onClick={handleReschedule} disabled={rSaving}
                    className="flex-grow bg-amber-500 hover:bg-amber-600 text-white py-3 px-6 rounded-xl text-sm font-bold transition disabled:opacity-50 flex items-center justify-center gap-2">
                    {rSaving
                      ? <><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Saving...</>
                      : <>📅 Confirm Reschedule</>
                    }
