...
// ─── requireHospitalAccess — hospital, admin, or staff with manageHospitals ──
// Returns { session } if caller may manage the given hospital, else { error }.
// hospitalMongoId: the hospital's _id string from the request body/query
export async function requireHospitalAccess(request, hospitalMongoId) {
  const session = await getSessionFromRequest(request);

  if (!session) {
    return {
      error: NextResponse.json({ success: false, message: "Login karein — session expire ho gaya" }, { status: 401 }),
      session: null,
    };
  }
  ...
  // Hospital: only their own hospital — verify ownership against JWT
  if (session.role === "hospital") {
    if (
      hospitalMongoId &&
      session.hospitalMongoId &&
      session.hospitalMongoId.toString() !== hospitalMongoId.toString()
    ) {
      return {
        error: NextResponse.json(
          { success: false, message: "Aap sirf apne hospital ko manage kar sakte hain" },
          { status: 403 }
        ),
        session: null,
      };
  ...
  // Staff: must have manageHospitals: true AND hospital in assignedHospitalIds
  if (session.role === "staff") {
    // We need the full user record to check permissions
    ...
    if (!user) {
      return { error: NextResponse.json({ success: false, message: "User not found" }, { status: 401 }), session: null };
    }
    const sp = user.staffPermissions || {};
    if (!sp.manageHospitals) {
      return { error: NextResponse.json({ success: false, message: "Hospital manage karne ki permission nahi hai" }, { status: 403 }), session: null };
    }
    if (hospitalMongoId) {
      const assigned = (sp.assignedHospitalIds || []).map((id) => id.toString());
      if (assigned.length > 0 && !assigned.includes(hospitalMongoId.toString())) {
        return { error: NextResponse.json({ success: false, message: "Is hospital ka access nahi hai" }, { status: 403 }), session: null };
      }
    }
    return { error: null, session };
  }
  ...
  return {
    error: NextResponse.json({ success: false, message: "Access denied" }, { status: 403 }),
    session: null,
  };
}
